...
class FileOperationsProvider():
    ...
    def find_file(self, configuration, directory, file):
        ...
        :param configuration: A FrameworkConfiguration instance
        ...
        for dir_path, directories, files in os.walk(directory, followlinks=True):
            for sub_dir in directories:
                self.find_file(
                    configuration, configuration.mounting_point + sub_dir, file)
    ...
    def create_img_file(self, logger, extension):
        ...
        creation_command = 'mkisofs -o /tmp/temp_image.{} {}'.format(
            extension, os.environ['DEVS_MOUNTPOINT'])
        process = subprocess.Popen(creation_command.split(
